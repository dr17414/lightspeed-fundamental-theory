...
pair mass 的 $2/5$。$N=6$ 的無 RNG 窮舉只用來驗證化約並證明 failure 並非
單一 antichain chamber，不能外推 registered $N$ 的 tail。四個 interval
avoidance counts 與五個 score-block concentration events 仍須逐
$(j,N,\theta)$ 交付不超過 $\beta_*$ 的解析或 validated bound；item 8 與
selector proof bucket 均維持 `OPEN`。
