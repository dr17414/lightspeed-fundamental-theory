...
| 8 | E5-D／E5-E multiplicity與 power | complete claim family＋lineage-wide spending／successor reserve＋power audit；numerical cap 須在不讀 arm data 下分別交付 (a) leakage／propagation 收緊後的 producer-bound feasibility 與完整 split certification-success probability 下界、(b) E2 null matched-cohort law 如何事前排除／控制 near-zero norm enclosure 的高相對寬度 regime 及其失敗機率；並交付 cap-conditioned matched-null law，不能把 conditional CLEAN power 當作完整 split power | `OPEN` |
