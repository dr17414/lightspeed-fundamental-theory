...
6a-E seed、不授權讀取任何 6a-S arm numerical field，也不授權 active wrong-support arm 判決，亦不完成 D.1
第 3 項或 Freeze-1a。
...
現行 source-of-truth 尚未給出一份可執行 6a-E preregistration。items 1–7 已
`CLOSED`；items 4／5 的 region-form delivery 經 PR #30 merge `f9c3a505` 與 state-only
closeout 記為 `CLOSED`。其中 items 3＋6
的共同交付物已由 PR #24 經獨立 review、CI 與 merge
完成，item 2 亦由 PR #26 經最終 head 獨立複核、CI 與 merge 完成。item 7 的 E4
source-of-record 已由 PR #28 經最終 head 獨立複核、CI、exact-head automated review 與
merge 完成。下列項目仍是 blocking：

1. E1／E2／E3 的 region forms 已由 PR #30 交付；power、multiplicity、fresh seed manifests
   與 ledger／runner schema 尚未固定。
...
`analysis/stage5c_planted_certification.py` 已由 PR #24 經獨立 review、CI 與 merge 完成，
closure item 3 因而為 `CLOSED`。這不授權 arm endpoint 求值。
...
| :--- | :--- | :---: |
| **E1 continuum contrast** | $S_\theta$ typed formula／prescription；paired 2D estimand；contrast orientation；joint metric；simultaneous region；effect floor；integration uncertainty；boundary equality rule | region form `CLOSED`；完整 gate 尚待 items 8–10 |
| **E2 target-null equivalence** | $T_+$ vs $T_+$、$T_-$ vs $T_-$ 的完整 same-pipeline generator／matching；2D equivalence region；TOST 或等價程序；兩 null arms 各自 cohort floor | region form `CLOSED`；完整 gate 尚待 items 8–10 |
| **E3 planted alternatives** | correct chiral、symmetric diffusion、sector blind 的完整參數域；active wrong-support typed intervention；Gate O；Gate E joint-law regions／逐分量規則；global swap exact-invariance arm | item 6 與 statistical-region form `CLOSED`；完整 gate 尚待 items 8–10 |
| **E4 distributional well-posedness** | fixed-$\epsilon$ continuum sequence；test-function topology；contact／box-boundary leakage；pairing existence；convergence rate／error bound；兩個獨立 implementations 與 agreement criterion | `CLOSED` |
...
| 2 | $\Pi_{\mathcal M}$ joint matched law | `STAGE5C_6A_E_JOINT_MATCHED_LAW.md`＋generator／matcher source-of-record＋paired covariance schema＋null calibration／falsifier suite | `CLOSED` |
...
| 4 | E1 joint contrast | `STAGE5C_6A_E_STATISTICAL_REGIONS.md`＋paired estimand／2D cluster-$t$ rectangle／joint floor／source-row-bound producer-sealed CLEAN item-3 provenance／exact-ensemble-fingerprinted aggregation-only numerical width／exact-dyadic outward uncertainty propagation／strict normalized boundary rules | `CLOSED` |
| 5 | E2 null equivalence | `STAGE5C_6A_E_STATISTICAL_REGIONS.md`＋two typed same-target null-pair specs／simultaneous interval-inclusion／strict equivalence margin／structural cohort floor | `CLOSED` |
...
| 8 | E5-D／E5-E multiplicity與 power | complete claim family＋lineage-wide spending／successor reserve＋power audit；numerical cap 須在不讀 arm data 下分別交付 (a) causal-boundary／cell-enclosure producer-bound feasibility 與完整 split certification-success probability 下界、(b) E2 null matched-cohort law 如何事前控制 near-zero norm／adaptive 誤差的高相對寬度及失敗機率，不能僅以 atom 數下界充數；並交付 cap-conditioned matched-null law，不能把 conditional CLEAN power 當作完整 split power。現行 `CertifiedEndpointPool` 要求原始 pool 逐列連續且全部 `CLEAN`，故 averaged-error 路徑必須先以確定性證明或事前 tail 協定關閉完整-pool failure gate；只分析有限 CLEAN error 的平均大小不能取代此義務。完整 iid $p_\theta$ support 已有 positive-measure selector-domain failure 與 selector 成功後的 E4 leakage failure set，故不得再以 pointwise／almost-sure deterministic `CLEAN` 作捷徑；這個 no-go 只證 $p_{\rm fail}>0$，不證其超過 $2.03\times10^{-6}$。合法交付須把 selector domain／selection、E4 input／structural／backend 與 item-3 certification failure 分層給出解析或 validated probability 上界，否則只能明文修訂 support、selector、item 7 或 items 3–5。已交付的 $R_{\rm geo}$ partial rule 以 $10^{-12}$ boundary／pair-separation margin，把最壞 $N=128$ 的 complement 界在 $3.3024\times10^{-8}$，並在 selector success 後確定關閉 input-schema、atom-cap、box／causal leakage；這不涵蓋九個可能 empty-selection 的 strata、adaptive backend 或 item-3 certification，故不得稱完整 Gate A closure。其內另有登記 $N\in\{64,96,128\}$、真實 `all_relations`／`links` selector-success witness：frozen 零-subdivision adaptive output 使 item-3 固定為 `NORM-INTERVAL-TOUCHES-ZERO`；這阻止在 $R_{\rm geo}$ 上直接宣稱 certification 確定通過，但單點不量化 failure tail，development-only 降低 `atol` 的 sensitivity 也不是 item-7 amendment。九個 selector strata 的承重義務已明確化為四個 permutation rectangle-occupancy avoidance tails 與五個 depth-score adjacent-block $2/5$ concentration tails；小 $N$ chamber counts 只驗證化約，不得外推 registered tail 或關閉 selector bucket。其 uniform-law proof targets 已以 exact-rational Hölder likelihood moments 取代粗糙的 pointwise $p_{\max}^N$ 損失，並事前凍結 integer powers 與向下取整 thresholds；這只把六個 $(N,\theta)$ targets 放寬到約 $2.30\times10^{-9}$ 至 $1.56\times10^{-8}$，不構成任何 avoidance／concentration bound，九個 strata 仍須逐一完成。其後須把 raw-pool factor、top-$M$ worst-subset factor 與 matching-conditioned factor 分帳，不得把某一 selection-agnostic bound 失效擴張為所有 matching 路線失效；在只知非負性、subset 與 $M_h\ge m_{\min}$ 時 factor 8 是 sharp，任何更小承重 factor 必須新增 error／feature／matched-index joint law 或 deterministic restriction。先按 selector $j$、$N$、target $\theta$、raw pool size 與匹配條件將 frozen E4 error 推前成 matched-null law，量化 cap 成功率；不得把 planted atom 或 item-2 抽象 endpoint covariance calibration 當成該分布。若以 diagnostic 估計 failure tail、mean、factor 或 $\Pr(G)$，必須**先獨立 review／merge** 固定 $N,L,H$、replications、diagnostic-only seed provenance／隔離、simultaneous uncertainty／資源限制、停止規則及 fail-closed 映射；mean／factor audit 必須逐座標使用 $x_k=e_k/D_{kk}$，screen 分帶 scalar $v=\max(x_1,x_2)$ 不得代入或與 per-coordinate 門檻比較；同 seed 精確 replay可供重現，新增樣本或延長須 amendment，未完成前不得生成 diagnostic inputs 或以任何 pilot 挑選 cap。item 7 的 8128-atom cap 在 $N\le128$ 自動滿足，在 $N>128$ 只依實際 selected pairs 判定，後者拒絕率須入 tail。若任一證明依賴改動 item 7 的 frozen enclosure levels、adaptive 容差或資源上限，必須先明文修訂已 `CLOSED` 的 item 7，完成獨立 review／CI 與 item 3／items 4–5 下游影響複核；若維持既有 E4，須另證其 cap 成功機率與 matched-null law 足夠 | `OPEN` |
| 9 | selection／confirmation manifests | fresh disjoint bases＋burn lifecycle＋earliest-unresolved successor mapping＋non-recycling；若 item-8 frozen E4 audit screen 協定經 review／merge，正式 manifest 須明文排除其專用診斷 seed range $[6{,}000{,}000{,}000,6{,}000{,}000{,}023]$；item 8 `CLOSED` 前，manifest／seed builder 不得 import／呼叫 `analysis.stage5c_e5_budget`，並須有拒絕未授權 allocation 的回歸 | `OPEN` |
| 10 | runner／ledger／adjudicator freeze | fail-closed executable implementation＋gate router 的 typed selector／support-mapping identity record＋swap runtime operation-trace certification＋end-to-end dress test＋committed predecessor-ledger prerequisite chain；任何新 namespace 的 authorization 必須重新 pin 當時 current runner 並由 CI 逐一比對，每一個正式執行會發生的 producer→adapter→consumer 資料轉手都須有一條使用真實兩端、不得以 mock 取代接縫的 deterministic end-to-end regression；item 8 `CLOSED` 前，runner／adjudicator 不得 import／呼叫 `analysis.stage5c_e5_budget`，並須有拒絕未授權 allocation 的回歸 | `OPEN` |
...
selector bucket 的後續純分析已把九個 empty events 化約為 permutation-order
問題：`interval_exact(m)` 需要控制不存在 rectangle occupancy $m$ 的 chamber
數；depth-band empty 必然伴隨 boundary／相鄰 score blocks 合計至少佔全部
pair mass 的 $2/5$。對 registered density 的轉移不再只用 pointwise
$p_{\max}^N$：事前固定 integer Hölder powers，把 uniform-event target 放寬至
$2.30\times10^{-9}$ 到 $1.56\times10^{-8}$，並以 exact-rational moment
regression 鎖定；這不是已取得的 tail bound。$N=6$ 的無 RNG 窮舉只用來
驗證化約並證明 failure 並非
單一 antichain chamber，不能外推 registered $N$ 的 tail。四個 interval
avoidance counts 與五個 score-block concentration events 仍須逐 registered
$(N,\theta)$ 符合對應 uniform target，或直接交付不超過 $\beta_*$ 的
target-law 解析／validated bound；item 8 與 selector proof bucket 均維持
`OPEN`。
...
item 2 已由 PR #26 交付；最終 head `f7dc1706` 經獨立複核確認 N1 的 structured
remaining-work 落點與一般 anti-drift 規則，CI #101 通過並以 merge commit `93b1cd19`
合併，故可單獨記為 `CLOSED`。該 item 的 falsifier 只認證 $|\rho|>0$ 時刪除 nonzero
cross-arm blocks 的失效；$\rho=0$ 未拒絕不認證 reference degrees of freedom，且 2048
replications 未針對 df-only 錯置配置檢定力。items 4／5 本次以
`STAGE5C_6A_E_STATISTICAL_REGIONS.md` 與解析 Student-reference oracle 提議補上該缺口；
固定 $B=32,m=192,p=2,\alpha=0.01$，且只把 reference df 從 $B-1=31$ 錯置為
$Bm-1=6143$；covariance／SE target 不變時，每座標解析 coverage 由
$0.9950000000000001$ 降為 $0.99145302805641$。final head `2d7f20e` 已由 Claude 對
main→head 完整樹獨立複核 GO、CI #126（345 passed／5 個既有 warnings）與 exact-head
automated review（無 major issues）確認，PR #30 以 expected-head squash-merge 為 main
`f9c3a505`，因此兩列在本 state-only closeout 記為 `CLOSED`。item 7 已由
`STAGE5C_6A_E_WELLPOSEDNESS.md`、`analysis/stage5c_e4_wellposedness.py` 與 regressions
交付；最終 head `81b926cc` 經獨立完整樹複核、CI #113、exact-head automated review 與
PR #28 merge `59c78c64` 完成，故可單獨記為 `CLOSED`。items 1–7 現為
`CLOSED`；items 8–10、12 仍為 `OPEN`、item 11 仍為 `DRAFT`，因而不解除任何
execution firewall。
...
items 1–7 已完成；remaining-work sequence 只包含 closure matrix 中尚未 `CLOSED`
的交付物：

1. 交付 item 8 的完整 claim family、lineage-wide multiplicity／successor reserve 與不讀 arm
   data 的 power audit；
2. 交付 item 9 的 fresh disjoint manifests、burn lifecycle 與 successor mapping；
3. 再凍結 item 10 的 runner、ledger、adjudicator與一次性 fresh lifecycle。runner 在建立 ledger、claim seed
...
4. 以 item 10 的實際 state machine 逐列封閉 item 11，最後完成 item 12 的整體獨立 review／CI／merge；
   只有 12 列全為 `CLOSED` 後，才可生成第一個 6a-E seed。

本次 state-only closeout 只把已獲獨立 GO、CI、automated review 與 merge 證據的 items 4／5
記為 `CLOSED`，不授權開啟現有 raw ledgers。若後續發現本 draft 漏列情形，
唯一合法動作是先修 draft；不是在執行時補判準。
