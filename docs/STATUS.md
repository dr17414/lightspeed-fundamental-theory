...
| **Stage 5C D.1-3 selector／endpoint contract 路線** | **【提案／v0.12 草案；C8 6a-S prereg 已交付，整體 feasibility 未證】** | distributional route 已接入 fork B、ambient norm、完整 complexified-real invariant algebra、C3b blind-variety distance form、二維 primary endpoint、wrong-direction gauge 邊界與 11 點 C8 intrinsic selector family。C8 numerical layer 已在不讀 contrast 下固定：$\varphi=1$、$\mathcal N_C=|\Sigma(C)|$、fixed-scale Gaussian regulator、80-mode Fourier random-measure signature、同 target S5 metric、S1/S6 floors、$N=64,96,128$、4×64 blocks 與 1536 個 reserved seeds；尚未生成或執行。6a-E joint-law metric／power／lifecycle、active wrong-support、C6/C7 instances，以及 C3b capability boundary、pair domain／weights、nontriviality／effect-size／noise／continuum calibration 仍未固定。D.1 第 3 項及 Freeze-1a 維持 PENDING，未設計 $K$。 | `docs/STAGE5C_D1_3_OBSERVABLE_CONTRACT.md`, `docs/STAGE5C_SELECTOR_FAMILY.md`, `docs/STAGE5C_6A_S_PREREGISTRATION.md`, `docs/STAGE5C_E3_WRONG_DIRECTION.md`, `docs/STAGE5C_D1_3_PRIMARY_INVARIANT.md`, `docs/STAGE5C_C3B_BLIND_DISTANCE.md`, `analysis/stage5c_measure_prereg.py`, `tests/test_stage5c_measure_prereg.py` |
| **Stage 5C C8 intrinsic selector family** | **【已確認之 candidate-independent 結構交付物／6a-S prereg 完成、execution PENDING】** | $\Sigma_{C8}$ 固定為 sector-blind、unweighted pair-set API，僅接受 `BlindedCase`；有序 family 11 點與 closed capacity 不變。numerical prereg 另固定 probability-measure normalization、linear regulator、finite-resolution mean／random-law metrics、coverage／ESS floors、failure semantics 與不相交 seed manifest。這只授權下一段依 commit 一次執行 contrast-free 6a-S；不得記為 6a-S PASS、SELECTOR-VIABLE 或 continuum convergence，亦不得直接進 6a-E。$\Delta r_U,\Delta r_V$ 仍不進 C8，active wrong-support 仍由獨立 L4 evaluator tuple 承擔。 | `docs/STAGE5C_SELECTOR_FAMILY.md`, `docs/STAGE5C_6A_S_PREREGISTRATION.md`, `analysis/stage5c_selector_family.py`, `analysis/stage5c_measure_prereg.py`, `tests/test_stage5c_selector_family.py`, `tests/test_stage5c_measure_prereg.py` |
