...
| **Stage 5C C8.4 定義域內困難對照可構造性** | **【matchability 已確認；residual discriminability 待執行檢定；Freeze-1a 未完成】** | 在不接觸任何候選 $K$ 下，建立同一 1+1D null square 上的 conformal-volume pair $p_{\pm0.4}(u,v)=1\pm0.4q(u)q(v)$、$q(z)=6z^2-6z+1$。兩側總體積與 null-coordinate marginals 相同，ordering fraction 期望值解析上精確同為 $1/2$，且每個有限樣本均有 order-dimension $\le2$。原 512 raw-pool 跨 seed 不穩，故提升為 768；用來選擇此設計點的 `910–943M` blocks 已降級為 exploratory，另以設計固定後的 `960–983M` 三組新 blocks 承重，三組皆通過 pairs／coverage／SMD／KS。$\kappa=1$ 在 768 亦得 343/732 對並通過全部 gate。新稽核同時發現 local density 差異會系統性洩漏進 global height／interval statistics，matching 對 oracle local contrast 的 like-for-like 衰減僅約 1.4%；但最帶 target 資訊的已知 order statistics 已被配平，因此尚未證明 matched cohort 保有 order-only residual power。在 candidate-independent reference probe 預登記、freeze 且取得判決前，本 control 不得用於候選評估。basis-invariant smearing／norm 與全域 multiplicity 亦仍待完成，故**不是 Freeze-1a 完成**。全程未設計候選 kernel；所有 seeds 均為 development-only，未揭露 candidate holdout。 | `docs/STAGE5C_C8_4_HARD_CONTROLS.md`, `analysis/stage5c_hard_controls.py`, `tests/test_stage5c_hard_controls.py`, `docs/stage5c_development_log.md` |
| **Stage 5C D.1-3 residual-discriminability reference probe** | **【已確認／預登記完成；尚未執行】** | 在任何 probe run 前固定兩層 candidate-independent 檢定：Layer 1 以 sealed coordinates 的 boundary-corrected、Jeffreys-smoothed local-density estimator 檢查 continuum contrast 是否仍在；Layer 2 以固定 43 維 order-only bank、calibration-only residualization 與 ridge score，執行 P（未匹配正對照）、N（同 target 等效性負對照）、Q（matched residual question）三 arms。資料 ranges、sample floor（含 arm N 的 n_N≥900 gate）、effect floor、Holm family、randomization streams 與失敗語意均已固定；刪除原草案的 binary 回流通道。執行結果只可公開 arm-level $n$/effect/$p$ 與判決，不能回流 feature 權重或成功族別。此交付物仍不是 C6/C7/C8 統一 object contract，也不構成 Freeze-1a。 | `docs/STAGE5C_D1_3_REFERENCE_PROBE.md` |
