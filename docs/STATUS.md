...
| **Stage 5C D.1-3 selector／endpoint contract 路線** | **【提案／起草路線；存在性未證】** | 後續 contract 暫採 distributional route：預先固定、candidate-independent、由 order+number 可計算的 selector $\Sigma$，並由 evaluator 端計算其 target-dependent induced distribution $\mu_\theta$，以取代要求離散側定位固定座標點 $s,t$。這只是起草方向：reference probe 僅證明固定 43 維 bank 可分辨 matched ensembles，**不證明**存在同時滿足 relabeling／sector covariance、two-point $K$-observable 對接、basis invariance 與可計算 $\mu_\theta$ 的合格 $\Sigma$。固定端點路線因目前缺少無 primitive 的 contract 而暫不採，但在沒有 completeness／no-go theorem 前**不得**宣稱原則上不可能。$\Sigma$ 的 form、容量、provenance、induced-distribution pipeline、effect／power 與 null controls 均須在候選 $K$ 之前完成 Freeze-1a；若明定搜尋類耗盡，只能記 BOUNDED-SEARCH-EXHAUSTED。原 $L_\theta$ 暫降為 continuum-oracle 動機／cross-check，只有替代 endpoint 通過完整 contract 才可正式取代。 | `docs/STAGE5C_D1_3_PROBE_RESULTS.md`, `docs/STAGE5C_ACCEPTANCE.md` |
