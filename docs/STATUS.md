...
| **Stage 5C D.1-3 selector／endpoint contract 路線** | **【提案／v0.3 草案；存在性與 feasibility 未證】** | distributional route 草案已建立。兩輪獨立 review 已撤回 trace／determinant invariant 生成、local bi-fiber similarity、pointwise nonlinear distribution、$\mu$ divergence 必要性與 constant-conformal-factor 不可觀測等過強說法。v0.3 進一步把 continuum target 固定為完整 candidate-independent random-measure law $\Pi_{\theta,g}$ 下的 mean-of-invariant；只有 concentration／continuity／uniform-integrability theorem 成立時才可用 invariant-of-mean 簡化，Freeze-2a 不得依 candidate 換端點。C8 使用含完整 matching lifecycle 與 paired covariance 的 joint $\Pi_{\mathcal M}$；6a-S 限於同 target、contrast-free batch prefilter，6a-E 才可讀 between-target contrast 並受原始 family 順序、fresh confirmation 與 family-wise spending 管制。E2 equivalence power 已與 E1/E3 detection power 分開；E3 oracle-built planted controls 的 PASS 對合規 order-only $K$ 存在性與 Stage 5C-1 可行性零蘊涵。reference probe 仍**不證明**合格 $\Sigma$ 存在，路線 A 仍未被否證。具體 $\mathcal F_\Sigma$、global fiber trivialization／basis group、invariant family、smearing／normalization、measure／regulator、planted alternatives、統計門檻與 lifecycle 均未固定；Freeze-1a 九項尚無任何一項可標記為完整 DELIVERED，故 D.1 第 3 項與 Freeze-1a 維持 PENDING。 | `docs/STAGE5C_D1_3_OBSERVABLE_CONTRACT.md`, `tests/test_stage5c_observable_contract.py`, `docs/STAGE5C_D1_3_PROBE_RESULTS.md`, `docs/STAGE5C_ACCEPTANCE.md` |
