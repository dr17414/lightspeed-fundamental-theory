...
| **Stage 5C C0–C11 acceptance specification** | **【已確認／規格定稿；Freeze-1a 未完成】** | 審計後經 Claude／GPT 多輪獨立交叉審查，v0.8 已把 C0–C11 升級為具 test／pass threshold／failure meaning／evidence artifact 的驗收契約，並封住 object-type 切換、realizer orbit 任選、L2/L3 容量偷渡、oracle 回流、候選自訂 blind space／projector／test space、事後放寬容差、跨 batch 多重嘗試、C4/C8 循環與候選層／規格層過強 No-Go 等漏洞。v0.8 另封住 C8.1 的新增-covariate 繞道：只允許 label-invariant global scalar，須在 candidate-independent qualification split 保持既定 cohort／power floor；把 local continuum contrast 配平者判 PROTOCOL-INVALID。規格定稿**不等於 Freeze-1a**：附錄 D.1 九項仍須在單一 freeze commit 對接完成；九項完成前不得設計候選 kernel、接觸 holdout 或開始 Stage 5C-1 confirmatory evaluation。全程未設計候選 kernel，handoff §2.3 的候選材料未帶入。 | `docs/STAGE5C_ACCEPTANCE.md`, `docs/STAGE5C_ACCEPTANCE_AUDIT.md`, `docs/handoff_v2.0.md` |
...
| **Stage 5C D.1-3 selector／endpoint contract 路線** | **【提案／v0.4 草案；存在性與 feasibility 未證】** | distributional route 草案已建立。前三輪獨立 review 已撤回 trace／determinant invariant 生成、local bi-fiber similarity、pointwise nonlinear distribution、$\mu$ divergence 必要性與 constant-conformal-factor 不可觀測等過強說法。v0.3 把 continuum target 固定為完整 candidate-independent random-measure law $\Pi_{\theta,g}$ 下的 mean-of-invariant；只有 concentration／continuity／uniform-integrability theorem 成立時才可用 invariant-of-mean 簡化，Freeze-2a 不得依 candidate 換端點。v0.4 再把 §1.4 群精確化為 fixed-slot sector-line stabilizer 與 declared structures 之交，區分一般被動 $GL(2)$ 換基，並引用獨立 PENDING-fork invariant analysis；A／B 尚未選定，非緊 full monomial fork 與 C3b continuous positive norm 有條件衝突。C8 使用含完整 matching lifecycle 與 paired covariance 的 joint $\Pi_{\mathcal M}$；6a-S 限於同 target、contrast-free batch prefilter，6a-E 才可讀 between-target contrast 並受原始 family 順序、fresh confirmation 與 family-wise spending 管制。E2 equivalence power 已與 E1/E3 detection power 分開；E3 oracle-built planted controls 的 PASS 對合規 order-only $K$ 存在性與 Stage 5C-1 可行性零蘊涵。reference probe 仍**不證明**合格 $\Sigma$ 存在，路線 A 仍未被否證。具體 $\mathcal F_\Sigma$、basis-group fork、$\mathfrak I_G$、smearing／normalization、measure／regulator、planted alternatives、統計門檻與 lifecycle 均未固定；Freeze-1a 九項尚無任何一項可標記為完整 DELIVERED，故 D.1 第 3 項與 Freeze-1a 維持 PENDING。 | `docs/STAGE5C_D1_3_OBSERVABLE_CONTRACT.md`, `docs/STAGE5C_D1_4_BASIS_GROUP.md`, `tests/test_stage5c_observable_contract.py`, `tests/test_stage5c_basis_group.py`, `docs/STAGE5C_D1_3_PROBE_RESULTS.md`, `docs/STAGE5C_ACCEPTANCE.md` |
| **Stage 5C §1.4 basis-group／invariant-family 分析** | **【已確認之代數分析／PENDING fork；尚未 freeze】** | 對目前 fixed-slot、裸 $M$ endpoint contract，群須定義為無序 sector-line stabilizer 與 pairing／reality／basis conventions 之交，故 $S_2\subseteq G\subseteq N=(\mathbb C^\times)^2\rtimes S_2$；一般 $GL(2)$ mixing 仍可作同時搬動 sector projectors 的被動座標變換，不能與 fixed-slot gauge action 混同。$S_2$ 全純 invariant ring 由 $a+d,ad,b+c,bc,(a-d)(b-c)$ 生成；full monomial 全純 ring 為 $\mathbb C[\operatorname{tr},\det,bc]$，所以 trace／det 均不完備。monomial 非閉軌道使 sector-blind／chiral 代表與其單側 triangular degeneration 無法由任何 continuous invariant 分離；同一 orbit-closure 論證亦排除完整矩陣 domain 上的 continuous positive $N$-invariant norm，故非緊 fork C 與 C3b norm gate 有條件衝突，不只是分離力較弱。$\Delta\ne0$ 只是不為純量的充分 witness，並非 sector non-degeneracy 的必要條件；$\Delta=0$ 不得單獨判 C3b FAIL。A（fixed relative basis）／B（swap-compatible positive Hermitian structure）尚未選定，其他中間 subgroup 亦須逐案重算；§1.4 與 D.1 第 3 項保持 PENDING。 | `docs/STAGE5C_D1_4_BASIS_GROUP.md`, `tests/test_stage5c_basis_group.py`, `docs/STAGE5C_D1_3_OBSERVABLE_CONTRACT.md` |
