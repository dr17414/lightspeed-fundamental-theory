...
| **Stage 5C D.1-3 selector／endpoint contract 路線** | **【提案／v0.12 草案；6a-S plus arm `PROTOCOL-INVALID`、Amendment-001 frozen、dress rehearsal PENDING】** | 1.3B plus 的 formal verdict 與永久 burned 狀態不變；1.4B 未 claim／未生成。Amendment-001 只修 runner implementation：刪除有因次 `kish_ess-selected` 的固定絕對比較，改以原已登記的無因次 ESS fraction relation／identity，並把 exact pure transform、dimensionless recomputation 與 theoretical identity reasons 分帳；沒有改任何 S1/S5/S6 threshold。fresh lifecycle 固定為 3.1B full-arm dress rehearsal → DEV-0012/hash attestation → 1.5B replacement plus → DEV-0013/hash attestation → 1.4B minus。四個承重 code files 的 digest 須跨 prerequisite／兩臂相同，claim 前 burn registry 也須是合法 lifecycle prefix 且目前 profile／target 尚未登記。下一步只可在 amendment review／merge 後執行 3.1B dress rehearsal；6a-E、active wrong-support、C6/C7 instances、D.1 第 3 項及 Freeze-1a 均維持 PENDING，未設計 $K$。 | `docs/STAGE5C_D1_3_OBSERVABLE_CONTRACT.md`, `docs/STAGE5C_SELECTOR_FAMILY.md`, `docs/STAGE5C_6A_S_PREREGISTRATION.md`, `docs/STAGE5C_6A_S_RUNNER_FREEZE.md`, `docs/STAGE5C_6A_S_EXECUTION_INCIDENT.md`, `docs/STAGE5C_6A_S_PROTOCOL_AMENDMENT_001.md`, `docs/stage5c_6a_s_burn_registry.json`, `docs/stage5c_protocol_amendment_log.md`, `docs/STAGE5C_E3_WRONG_DIRECTION.md`, `analysis/stage5c_measure_prereg.py`, `analysis/stage5c_6a_s_runner.py`, `tests/test_stage5c_6a_s_runner.py` |
| **Stage 5C C8 intrinsic selector family** | **【已確認之 candidate-independent 結構交付物／6a-S replacement pre-execution】** | $\Sigma_{C8}$ 的 11 點 family、selector 定義與 numerical gates 均未改。正式 1.3B plus ledger 維持五點單臂 PASS／六點 protocol-invalid，且不得外推為 selector viability。Amendment-001 沒有用 incident 最大誤差設定新 tolerance；它只以事故前已固定的無因次 ESS fraction contract 修正 adjudicator，另要求真實規模的 3.1B end-to-end dress rehearsal。跨臂 code identity 由四檔 digest 恢復保證，已 committed profile／target 則由 burn registry 在 claim 前拒絕重跑。1.5B replacement plus 與原 1.4B minus 尚未生成；沒有 target contrast 或 combined verdict。任何 reserved 續跑前必須依序滿足 committed attestation，不得直接進 6a-E。 | `docs/STAGE5C_SELECTOR_FAMILY.md`, `docs/STAGE5C_6A_S_PREREGISTRATION.md`, `docs/STAGE5C_6A_S_RUNNER_FREEZE.md`, `docs/STAGE5C_6A_S_EXECUTION_INCIDENT.md`, `docs/STAGE5C_6A_S_PROTOCOL_AMENDMENT_001.md`, `docs/stage5c_6a_s_burn_registry.json`, `analysis/stage5c_selector_family.py`, `analysis/stage5c_measure_prereg.py`, `analysis/stage5c_6a_s_runner.py`, `tests/test_stage5c_6a_s_runner.py` |
