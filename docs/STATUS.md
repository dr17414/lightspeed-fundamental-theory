...
| **Stage 5C D.1-3 selector／endpoint contract 路線** | **【提案／v0.8 草案；存在性與 feasibility 未證】** | distributional route 的 continuum law、matching lifecycle、fork B 與 ambient Frobenius norm 均已固定到草案層。v0.8 接入完整 complexified-real invariant algebra與 C3b rank-one blind-variety distance form。此交付物尚未選 primary $\mathfrak I_G$；selector／measure／smearing、planted thresholds 與 lifecycle，以及 C3b capability boundary、pair domain／weights、nontriviality／effect-size／noise／continuum calibration 仍未固定。D.1 第 3 項及 Freeze-1a 維持 PENDING，未設計 $K$。 | `docs/STAGE5C_D1_3_OBSERVABLE_CONTRACT.md`, `docs/STAGE5C_C3B_BLIND_DISTANCE.md`, `docs/STAGE5C_D1_4_BASIS_GROUP.md`, `docs/STAGE5C_D1_4_INVARIANT_ALGEBRA.md`, `tests/test_stage5c_observable_contract.py`, `tests/test_stage5c_blind_distance.py`, `tests/test_stage5c_basis_group.py`, `tests/test_stage5c_invariant_algebra.py` |
| **Stage 5C §1.4 basis-group／invariant-family 分析** | **【已確認之代數分析／fork B、ambient norm、invariant algebra 已交付；尚未 freeze】** | fixed-slot 群經一般 noncompact orbit-closure no-go 與 C0 最小 primitive 紀律定為 $h=p\mathbb 1$、$G=T^2\rtimes S_2$、$G_{\rm eff}=U(1)\rtimes S_2$；pairing／adjoint 是 L2，整體尺度才屬 L3 normalization。Frobenius norm 顯式 discharge ambient existence。後續 invariant-algebra review 補入遺漏的 $Q=|a-d|^2$，區分 ring generation 與 orbit separation，並完成 toric／second-Veronese 解析生成證明。holomorphic-only information 與 noncompact complexification 的 quotient 相同，會保留 triangular degeneration blind spots；但這不表示群真的改回 fork C，也不使 Frobenius norm 消失。primary $\mathfrak I_G$ 與 C3b 未完成的 capability／calibration 項仍 PENDING；未設計 $K$。 | `docs/STAGE5C_D1_4_BASIS_GROUP.md`, `docs/STAGE5C_D1_4_INVARIANT_ALGEBRA.md`, `tests/test_stage5c_basis_group.py`, `tests/test_stage5c_invariant_algebra.py` |
