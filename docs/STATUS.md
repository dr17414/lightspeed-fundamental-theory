...
| **Stage 5C D.1-3 selector／endpoint contract 路線** | **【提案／v0.5 草案；存在性與 feasibility 未證】** | distributional route 草案已建立。前三輪 review 已修正 trace／determinant invariant 生成、local bi-fiber similarity、pointwise nonlinear distribution、$\mu$ divergence 必要性與 constant-conformal-factor 等過強說法；continuum target 固定為 candidate-independent random-measure law，C8 使用完整 matching lifecycle 的 joint $\Pi_{\mathcal M}$。v0.5 依 C3b 的連續正定 norm 與 C0 最小 primitive／完整 stabilizer 兩項具名前提，選定 fork B：$h=p\mathbb 1$、$G=T^2\rtimes S_2$、$G_{\rm eff}=U(1)\rtimes S_2$。$h/\mathbb R_{>0}$ 明列 declared L2 pairing scaffold；$p$ 吸收進 L3 normalization，不列 L4。此子決議不選 $\mathfrak I_G$，亦不證明合格 $\Sigma$ 或 order-only $K$ 存在。具體 $\mathcal F_\Sigma$、primary real invariant、smearing／normalization、measure／regulator、planted alternatives、統計門檻與 lifecycle 仍未固定；D.1 第 3 項及 Freeze-1a 維持 PENDING。 | `docs/STAGE5C_D1_3_OBSERVABLE_CONTRACT.md`, `docs/STAGE5C_D1_4_BASIS_GROUP.md`, `tests/test_stage5c_observable_contract.py`, `tests/test_stage5c_basis_group.py`, `docs/STAGE5C_D1_3_PROBE_RESULTS.md`, `docs/STAGE5C_ACCEPTANCE.md` |
| **Stage 5C §1.4 basis-group／invariant-family 分析** | **【已確認之代數分析／fork B 已定案；尚未 freeze】** | fixed-slot 群滿足 $S_2\subseteq G\subseteq N=(\mathbb C^\times)^2\rtimes S_2$。review 提議的 closed-subgroup 清單不完整（離散 radial spiral 即反例），故決議改以一般 orbit-closure 論證承重：任一非緊有效 $H\le\mathbb C^\times$ 均使 nilpotent 軌道逼近零，與 C3b 的全域連續正定 invariant norm 不相容。再以 C0 最小 primitive 紀律要求只宣告 norm 所需結構並取完整 stabilizer；一般 swap-compatible Hermitian form $h=\left(\begin{smallmatrix}p&q\\q&p\end{smallmatrix}\right)$ 的 $q\ne0$ 會偷渡 relative-phase primitive，故 $h=p\mathbb 1$，得到 $G=T^2\rtimes S_2$、$G_{\rm eff}=U(1)\rtimes S_2$。pairing／adjoint 依 frozen ledger 是 **L2**，不是 L3；只有吸收進 $\mathcal N$ 的整體尺度屬 L3 normalization。此決議具名依賴 C3b norm 型別與 C0 最小結構，未選 primary invariant、未設計 $K$；D.1 第 3 項仍 PENDING。 | `docs/STAGE5C_D1_4_BASIS_GROUP.md`, `tests/test_stage5c_basis_group.py`, `docs/STAGE5C_D1_3_OBSERVABLE_CONTRACT.md` |
