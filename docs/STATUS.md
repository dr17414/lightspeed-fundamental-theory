...
| **Stage 5C D.1-3 selector／endpoint contract 路線** | **【提案／v0.10 草案；primary 已選，整體 feasibility 未證】** | distributional route 已接入 fork B、ambient norm、完整 complexified-real invariant algebra、C3b blind-variety distance form、二維 primary endpoint 與 wrong-direction gauge 邊界。全域 sector swap 已確認只能作 invariance arm；active wrong-support control 仍須由完整 typed selector tuple 定義並依序通過 orbit-admissibility 與 endpoint-law separation。selector／measure／smearing／normalization、joint-law metric、完整 planted controls／thresholds 與 lifecycle，以及 C3b capability boundary、pair domain／weights、nontriviality／effect-size／noise／continuum calibration 仍未固定。D.1 第 3 項及 Freeze-1a 維持 PENDING，未設計 $K$。 | `docs/STAGE5C_D1_3_OBSERVABLE_CONTRACT.md`, `docs/STAGE5C_E3_WRONG_DIRECTION.md`, `docs/STAGE5C_D1_3_PRIMARY_INVARIANT.md`, `docs/STAGE5C_C3B_BLIND_DISTANCE.md`, `docs/STAGE5C_D1_4_BASIS_GROUP.md`, `docs/STAGE5C_D1_4_INVARIANT_ALGEBRA.md`, `analysis/stage5c_primary_invariant.py`, `tests/test_stage5c_primary_invariant.py`, `tests/test_stage5c_wrong_direction.py` |
