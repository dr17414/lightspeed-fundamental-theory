...
| **Stage 5C D.1-3 selector／endpoint contract 路線** | **【提案／v0.7 草案；存在性與 feasibility 未證】** | distributional route 的 continuum law、matching lifecycle、fork B 與 ambient Frobenius norm 均已固定到草案層。v0.7 再接入完整 complexified-real invariant algebra：$A=a+d,P=ad,W=bc,S=|b|^2+|c|^2,Q=|a-d|^2,R=(a-d)(|b|^2-|c|^2)$ 及共軛；final relation ideal 由 symmetric rank-one $3\times3$ matrix 的六個 $2\times2$ minors 生成。此交付物只給可用 algebra 與 holomorphic-only 禁限，尚未選 primary $\mathfrak I_G$；blind-space quotient／projection、aggregate norm、selector／measure／smearing、planted thresholds 與 lifecycle 仍未固定。D.1 第 3 項及 Freeze-1a 維持 PENDING，未設計 $K$。 | `docs/STAGE5C_D1_3_OBSERVABLE_CONTRACT.md`, `docs/STAGE5C_D1_4_BASIS_GROUP.md`, `docs/STAGE5C_D1_4_INVARIANT_ALGEBRA.md`, `tests/test_stage5c_observable_contract.py`, `tests/test_stage5c_basis_group.py`, `tests/test_stage5c_invariant_algebra.py` |
| **Stage 5C §1.4 basis-group／invariant-family 分析** | **【已確認之代數分析／fork B、ambient norm、invariant algebra 已交付；尚未 freeze】** | fixed-slot 群經一般 noncompact orbit-closure no-go 與 C0 最小 primitive 紀律定為 $h=p\mathbb 1$、$G=T^2\rtimes S_2$、$G_{\rm eff}=U(1)\rtimes S_2$；pairing／adjoint 是 L2，整體尺度才屬 L3 normalization。Frobenius norm 顯式 discharge ambient existence。後續 invariant-algebra review 補入遺漏的 $Q=|a-d|^2$，區分 ring generation 與 orbit separation，並完成 toric／second-Veronese 解析生成證明。holomorphic-only information 與 noncompact complexification 的 quotient 相同，會保留 triangular degeneration blind spots；但這不表示群真的改回 fork C，也不使 Frobenius norm 消失。primary $\mathfrak I_G$ 與 C3b quotient contract 仍 PENDING；未設計 $K$。 | `docs/STAGE5C_D1_4_BASIS_GROUP.md`, `docs/STAGE5C_D1_4_INVARIANT_ALGEBRA.md`, `tests/test_stage5c_basis_group.py`, `tests/test_stage5c_invariant_algebra.py` |
| **Stage 5C §1.4 fork-B invariant algebra** | **【已確認／完整生成與 relation ideal 已交付；primary 尚未選】** | torus 半群 Hilbert basis 為 $u=|b|^2,v=|c|^2,W=bc,\bar W$，其 toric kernel 唯一關係 $uv=W\bar W$。取 swap 後，完整 complexified-real ring 由 $A,P,W,S,Q,R$ 及共軛生成；$Q$ 對 ring generation 必要、對點值 separation 不必要。final presentation 有六條 second-Veronese minors；上游 $uv=W\bar W$ 已在消去 $u,v$ 時吸收，不能誤算為第七條 final syzygy。625 點結構化格點的正確 unordered-pair 計數為全純 2950／跨完整 orbit 1000，實族 1950／跨完整 orbit 0；此只作 regression，不代替全域解析證明。E3 承重 primary 必須含實不變量，但單一 $S$ 只保證分離兩組具名 degeneration pairs。 | `docs/STAGE5C_D1_4_INVARIANT_ALGEBRA.md`, `tests/test_stage5c_invariant_algebra.py` |
