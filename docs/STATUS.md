...
| **Stage 5C D.1-3 selector／endpoint contract 路線** | **【提案／v0.11 草案；primary 與 C8 selector 結構已選，整體 feasibility 未證】** | distributional route 已接入 fork B、ambient norm、完整 complexified-real invariant algebra、C3b blind-variety distance form、二維 primary endpoint、wrong-direction gauge 邊界與 11 點 C8 intrinsic selector family。全域 sector swap 只能作 invariance arm；active wrong-support 須由獨立 L4 evaluator tuple 定義，不要求 C8 selector sector-aware。selector 的完整 6a-S prereg、measure／smearing／normalization、joint-law metric、完整 planted controls／thresholds 與 lifecycle，以及 C3b capability boundary、pair domain／weights、nontriviality／effect-size／noise／continuum calibration 仍未固定。D.1 第 3 項及 Freeze-1a 維持 PENDING，未設計 $K$。 | `docs/STAGE5C_D1_3_OBSERVABLE_CONTRACT.md`, `docs/STAGE5C_SELECTOR_FAMILY.md`, `docs/STAGE5C_E3_WRONG_DIRECTION.md`, `docs/STAGE5C_D1_3_PRIMARY_INVARIANT.md`, `docs/STAGE5C_C3B_BLIND_DISTANCE.md`, `docs/STAGE5C_D1_4_BASIS_GROUP.md`, `docs/STAGE5C_D1_4_INVARIANT_ALGEBRA.md`, `analysis/stage5c_selector_family.py`, `tests/test_stage5c_selector_family.py` |
| **Stage 5C C8 intrinsic selector family** | **【已確認之 candidate-independent 結構交付物／完整 prereg 與 6a-S PENDING】** | $\Sigma_{C8}$ 固定為 sector-blind、unweighted pair-set API，僅接受 `BlindedCase`。有序 family 共 11 個參數點：`all_relations`、`links`、`interval_exact(1..4)`、五個既有 height-CDF endpoints 的 `source_depth_band`；closed capacity 恰為 11，無事後填補空位。原草案的裸 ndarray firewall 缺口、錯誤 relabel inverse、無 provenance coverage cap、concentration 必要條件與 reference-probe-derived valency member 均已撤回。$\Delta r_U,\Delta r_V$ 仍是未採納 candidate materials，不進 C8；active wrong-support 改由獨立 L4 evaluator tuple 承擔。新增 13 項純結構 regression，但尚未凍結 $\varphi/\mathcal N$、measure metric、ESS、數值門檻與 seeds，故不得記 6a-S PASS。 | `docs/STAGE5C_SELECTOR_FAMILY.md`, `analysis/stage5c_selector_family.py`, `tests/test_stage5c_selector_family.py`, `docs/STAGE5C_D1_3_OBSERVABLE_CONTRACT.md` |
