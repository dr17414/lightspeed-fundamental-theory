...
| **Stage 5C D.1-3 selector／endpoint contract 路線** | **【提案／v0.12 草案；6a-S plus arm `PROTOCOL-INVALID`、amendment PENDING】** | runner freeze 合併後依 `3bdc062a` 首次執行 plus arm；768 個 1.3B seeds 已 write-ahead claim、生成並永久 burned，完整 ledger 已封存。凍結 adjudicator 因 uniform-weight Kish ESS 的浮點 roundoff 超過共用 $10^{-12}$ 絕對容差，把 `all_relations` 與五個 depth bands 判為 `PROTOCOL-INVALID`；這不是 S1/S5/S6 科學 FAIL，也不得事後改判。minus 1.4B manifest 未 claim、未生成，故不存在 combined ledger 或 final 6a-S verdict。下一步必須先以獨立 protocol amendment 固定尺度穩健的 ESS identity check 與 fresh seed lifecycle；6a-E、active wrong-support、C6/C7 instances、D.1 第 3 項及 Freeze-1a 均維持 PENDING，未設計 $K$。 | `docs/STAGE5C_D1_3_OBSERVABLE_CONTRACT.md`, `docs/STAGE5C_SELECTOR_FAMILY.md`, `docs/STAGE5C_6A_S_PREREGISTRATION.md`, `docs/STAGE5C_6A_S_RUNNER_FREEZE.md`, `docs/STAGE5C_6A_S_EXECUTION_INCIDENT.md`, `docs/STAGE5C_E3_WRONG_DIRECTION.md`, `analysis/stage5c_measure_prereg.py`, `analysis/stage5c_6a_s_runner.py`, `tests/test_stage5c_6a_s_runner.py` |
| **Stage 5C C8 intrinsic selector family** | **【已確認之 candidate-independent 結構交付物／6a-S execution protocol-invalid】** | $\Sigma_{C8}$ 的 11 點 family、selector 定義與 numerical gates 均未改。正式 plus arm 的 768 個 1.3B seeds 已 burned；凍結 adjudicator 對 Kish ESS 理論整數恆等式採共用 $10^{-12}$ 絕對容差，因最高 $1.86\times10^{-11}$ 的浮點 roundoff 產生 protocol-invalid。`links` 與四個 `interval_exact` 的單臂 categorical verdict 為 PASS，其餘六點為 protocol-invalid；不得把此單臂輸出外推為 selector viability。minus 1.4B 未生成，沒有 target contrast 或 combined verdict。任何續跑前須先完成、review 並合併 protocol amendment；不得直接進 6a-E。 | `docs/STAGE5C_SELECTOR_FAMILY.md`, `docs/STAGE5C_6A_S_PREREGISTRATION.md`, `docs/STAGE5C_6A_S_RUNNER_FREEZE.md`, `docs/STAGE5C_6A_S_EXECUTION_INCIDENT.md`, `analysis/stage5c_selector_family.py`, `analysis/stage5c_measure_prereg.py`, `analysis/stage5c_6a_s_runner.py`, `tests/test_stage5c_6a_s_runner.py` |
...
---
*狀態頁更新記錄：*
*v1.36 (2026-09-02) - 依 runner freeze merge commit `3bdc062a` 首次執行 reserved 6a-S plus arm。全部 768 個 1.3B seeds 已 durable claim、生成並永久 burned；ledger counts 與 SHA-256 hash chain 完整。凍結 adjudicator 因 uniform-weight Kish ESS roundoff（最大約 $1.86\times10^{-11}$）超過共用 $10^{-12}$ 絕對容差，正式輸出六點 `PROTOCOL-INVALID`、五點單臂 PASS；這是 runner identity-check 的尺度問題，不是 scientific gate FAIL，但不得事後重判。發現後停止，1.4B minus manifest 未 claim／未生成，沒有 combined verdict、target numerical contrast、primary endpoint 或候選 $K$。6a-S 未完成；下一步改為先做顯式 protocol amendment 與 fresh seed lifecycle。*
