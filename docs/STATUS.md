...
| **Stage 5C D.1-3 selector／endpoint contract 路線** | **【提案／v0.2 草案；存在性與 feasibility 未證】** | distributional route 草案已建立，但獨立 review 修掉五個 blocking 過強／型別錯誤：$G\ni\sigma_x$ 不推出 trace／determinant 生成全部 invariants；bi-fiber map 若容許 local basis change 不是單一 similarity；continuum $G_R$ 是 distribution，必須先 linear smear 再取 invariant；$\mu_+\ne\mu_-$ 既非必要亦不足以證明完整 endpoint 有 power；常數 conformal factor 不能因「order+number 不可觀測」而被強制消去。修正版以 weighted pair measure 對接 continuum；C8 明定使用含完整 matching lifecycle 與 paired covariance 的 $\bar\nu_{\theta\mid\mathcal M}$，不得以 raw marginals 代替。另要求逐成員 L2/L3 capacity ledger、fresh selector-confirmation split，以及候選前的 continuum contrast／null／planted-alternative／distributional-well-posedness／power tests。reference probe 仍**不證明**合格 $\Sigma$ 存在，路線 A 仍未被否證。具體 $\mathcal F_\Sigma$、global fiber trivialization／basis group、invariant family、smearing／normalization、measure／regulator、planted alternatives、統計門檻與 lifecycle 均未固定，故 D.1 第 3 項與 Freeze-1a 維持 PENDING。 | `docs/STAGE5C_D1_3_OBSERVABLE_CONTRACT.md`, `tests/test_stage5c_observable_contract.py`, `docs/STAGE5C_D1_3_PROBE_RESULTS.md`, `docs/STAGE5C_ACCEPTANCE.md` |
