...
| **Stage 5C D.1-3 selector／endpoint contract 路線** | **【提案／v0.6 草案；存在性與 feasibility 未證】** | distributional route 草案已建立。前三輪 review 已修正 trace／determinant invariant 生成、local bi-fiber similarity、pointwise nonlinear distribution、$\mu$ divergence 必要性與 constant-conformal-factor 等過強說法；continuum target 固定為 candidate-independent random-measure law，C8 使用完整 matching lifecycle 的 joint $\Pi_{\mathcal M}$。依 C3b 的連續正定 norm 與 C0 最小 primitive／完整 stabilizer，已選定 fork B：$h=p\mathbb 1$、$G=T^2\rtimes S_2$、$G_{\rm eff}=U(1)\rtimes S_2$；$h/\mathbb R_{>0}$ 是 declared L2 pairing scaffold，$p$ 吸收進 L3 normalization。v0.6 以 $\|M\|_F=\sqrt{\operatorname{tr}(MM^\dagger)}$ 構造性 discharge ambient-norm existence，但 blind-space quotient／projection 與 aggregate norm 仍未固定。此子決議不選 $\mathfrak I_G$，亦不證明合格 $\Sigma$ 或 order-only $K$ 存在；D.1 第 3 項及 Freeze-1a 維持 PENDING。 | `docs/STAGE5C_D1_3_OBSERVABLE_CONTRACT.md`, `docs/STAGE5C_D1_4_BASIS_GROUP.md`, `tests/test_stage5c_observable_contract.py`, `tests/test_stage5c_basis_group.py`, `docs/STAGE5C_D1_3_PROBE_RESULTS.md`, `docs/STAGE5C_ACCEPTANCE.md` |
| **Stage 5C §1.4 basis-group／invariant-family 分析** | **【已確認之代數分析／fork B 與 ambient norm 已定案；尚未 freeze】** | fixed-slot 群滿足 $S_2\subseteq G\subseteq N=(\mathbb C^\times)^2\rtimes S_2$。closed-subgroup 簡化清單不完整，故以一般 orbit-closure 論證排除任一非緊有效 $H\le\mathbb C^\times$。C0 最小 primitive 紀律再排除會偷渡 relative-phase primitive 的 Hermitian cross term，得到 $h=p\mathbb 1$、$G=T^2\rtimes S_2$、$G_{\rm eff}=U(1)\rtimes S_2$；pairing／adjoint 是 L2，只有整體尺度屬 L3 normalization。命題 6 顯式證明 Frobenius norm 在完整 $M_2(\mathbb C)$ 上連續、正定且 $G$-不變，故 P1 的 ambient existence 已 discharge；但 C3b quotient contract 與 primary real invariant family 尚未交付。未選 $\mathfrak I_G$、未設計 $K$；D.1 第 3 項仍 PENDING。 | `docs/STAGE5C_D1_4_BASIS_GROUP.md`, `tests/test_stage5c_basis_group.py`, `docs/STAGE5C_D1_3_OBSERVABLE_CONTRACT.md` |
