...
| **Stage 5C D.1-3 selector／endpoint contract 路線** | **【提案／v0.12 草案；1.5B replacement plus 11/11 categorical PASS、DEV-0013 review PENDING】** | 1.3B plus 的 formal protocol-invalid verdict 與永久 burned 狀態不變；DEV-0012 已合併。依 main `1e867e7a` 一次執行的 1.5B replacement plus 已完成：768 claims／samples、11 selectors 與 198 block pairs 的完整 ledger 通過 hash-chain／schema 重驗，此 arm 自身 categorical adjudication 為 11/11 `6a-S PASS`、0 `PROTOCOL-INVALID`、0 `INCONCLUSIVE`。不引用 DEV-0012 pattern 作解釋或重判；這仍是單臂 verdict，不是 target contrast、combined verdict 或 selector viability 結論。1.5B 全部永久 burned，raw ledger 未獲明確公開授權。DEV-0013、burn registry 與 plus attestation 須先經 CI、獨立 review／merge，1.4B minus 才能解鎖；若四檔或 runtime 漂移只能走 AMEND-0002＋fresh dress profile／base。6a-E、active wrong-support、C6/C7 instances、D.1 第 3 項及 Freeze-1a 均維持 PENDING，未設計 $K$。 | `docs/STAGE5C_D1_3_OBSERVABLE_CONTRACT.md`, `docs/STAGE5C_SELECTOR_FAMILY.md`, `docs/STAGE5C_6A_S_PREREGISTRATION.md`, `docs/STAGE5C_6A_S_RUNNER_FREEZE.md`, `docs/STAGE5C_6A_S_EXECUTION_INCIDENT.md`, `docs/STAGE5C_6A_S_PROTOCOL_AMENDMENT_001.md`, `docs/stage5c_6a_s_burn_registry.json`, `docs/stage5c_6a_s_dress_rehearsal_attestation.json`, `docs/stage5c_6a_s_replacement_plus_attestation.json`, `docs/stage5c_development_log.md`, `docs/STAGE5C_E3_WRONG_DIRECTION.md`, `analysis/stage5c_measure_prereg.py`, `analysis/stage5c_6a_s_runner.py`, `tests/test_stage5c_6a_s_runner.py` |
| **Stage 5C C8 intrinsic selector family** | **【已確認之 candidate-independent 結構交付物／1.5B plus categorical PASS、minus pre-execution】** | $\Sigma_{C8}$ 的 11 點 family、selector 定義與 numerical gates 均未改。正式 1.3B plus ledger 維持原 verdict，不重判；DEV-0012 rehearsal 只證 runner mechanically clean。1.5B replacement plus 自身 categorical adjudication 為 11/11 PASS、無 protocol-invalid／inconclusive；不得以 3.1B pattern 解釋、校正或加強它。跨臂 code/runtime identity、burn registry 與 committed attestations 仍強制 lifecycle；1.4B minus 尚未生成，沒有 target contrast 或 combined verdict。DEV-0013 commit 未經獨立 review／merge 前不得啟動 minus。 | `docs/STAGE5C_SELECTOR_FAMILY.md`, `docs/STAGE5C_6A_S_PREREGISTRATION.md`, `docs/STAGE5C_6A_S_RUNNER_FREEZE.md`, `docs/STAGE5C_6A_S_EXECUTION_INCIDENT.md`, `docs/STAGE5C_6A_S_PROTOCOL_AMENDMENT_001.md`, `docs/stage5c_6a_s_burn_registry.json`, `docs/stage5c_6a_s_replacement_plus_attestation.json`, `analysis/stage5c_selector_family.py`, `analysis/stage5c_measure_prereg.py`, `analysis/stage5c_6a_s_runner.py`, `tests/test_stage5c_6a_s_runner.py` |
...
---
*狀態頁更新記錄：*
*v1.41 (2026-09-04) - 依 main `1e867e7a86640afd963f362482acf69ef3605823` 與 frozen runner v2 一次執行 1.5B replacement-reserved plus。全部 768 seeds 均先 durable claim 再生成並永久 burned；外部 NDJSON 為 6,843,989 bytes、SHA-256 `1e515ca81c3e32fb11ff7e3a90d810a1e5264ce5217fc722775f3765165696cc`，hash chain／schema 重驗通過。10,196 records 完整；本 arm 自身 categorical adjudication 為 11/11 `6a-S PASS`、0 `PROTOCOL-INVALID`、0 `INCONCLUSIVE`。header 的 protocol digest／start-registry hash 為 `1343eb8e…29707`／`cb18709d…abda`，runtime 與 DEV-0012 逐字相同。未引用 3.1B pattern 解釋或重判，未公開或討論 per-causet／block-pair 數值；沒有 target contrast、combined verdict、primary endpoint 或候選 $K$。DEV-0013／registry／plus attestation 尚待 CI 與獨立 review／merge；之前不得 claim 1.4B。*
