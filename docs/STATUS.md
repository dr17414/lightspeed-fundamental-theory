...
| **Stage 5C D.1-3 selector／endpoint contract 路線** | **【提案／v0.12 草案；C8 6a-S runner 已凍結、execution PENDING】** | distributional route 已接入 fork B、ambient norm、完整 complexified-real invariant algebra、C3b blind-variety distance form、二維 primary endpoint、wrong-direction gauge 邊界與 11 點 C8 intrinsic selector family。C8 numerical layer 已在不讀 contrast 下固定：$\varphi=1$、$\mathcal N_C=|\Sigma(C)|$、fixed-scale Gaussian regulator、40 complex／80 real nonredundant Fourier signature、同 target S5 metric、S1/S6 floors、$N=64,96,128$、4×64 blocks 與 1536 個 reserved seeds；尚未生成或執行。DEV-0008/0009 已修正 sparse member、V-bias、conjugate redundancy 與 claim scope；DEV-0010 再登記 $d_{\rm mean}$ binding cell，並固定 one-arm-only runner、write-ahead burned-seed ledger、contrast firewall 與重算 S1–S6 的 adjudicator。6a-E joint-law metric／power／lifecycle、active wrong-support、C6/C7 instances，以及 C3b capability boundary、pair domain／weights、nontriviality／effect-size／noise／continuum calibration 仍未固定。D.1 第 3 項及 Freeze-1a 維持 PENDING，未設計 $K$。 | `docs/STAGE5C_D1_3_OBSERVABLE_CONTRACT.md`, `docs/STAGE5C_SELECTOR_FAMILY.md`, `docs/STAGE5C_6A_S_PREREGISTRATION.md`, `docs/STAGE5C_6A_S_RUNNER_FREEZE.md`, `docs/STAGE5C_E3_WRONG_DIRECTION.md`, `analysis/stage5c_measure_prereg.py`, `analysis/stage5c_6a_s_runner.py`, `tests/test_stage5c_6a_s_runner.py` |
| **Stage 5C C8 intrinsic selector family** | **【已確認之 candidate-independent 結構交付物／6a-S runner freeze 完成、execution PENDING】** | $\Sigma_{C8}$ 固定為 sector-blind、unweighted pair-set API，僅接受 `BlindedCase`；有序 family 與 closed capacity 維持 11。原 `source_depth_band` 已在 reserved streams 未生成前改為 order-dual `endpoint_depth_mass_band`；五點與 grid 不變。S5 使用 signed unbiased U-statistic 與 40 complex half-shell；$d_{\rm law}$ 的 0.20 gate 只作 gross-breakage tripwire。DEV-0009/0010 分別登記 `interval_exact(4)`, $N=64$ floor-binding cell 與 `endpoint_depth_mass_band(0.8,1.0)`, $N=64$ 的 $d_{\rm mean}$ binding cell，不改任何 gate、family 或 capacity。runner 以單臂 ledger 隔離數值，跨臂只合併 categorical verdicts。下一段只可依 runner freeze 合併後的 main commit 一次執行 6a-S；不得預記為 PASS、SELECTOR-VIABLE、law stability 或 continuum convergence。 | `docs/STAGE5C_SELECTOR_FAMILY.md`, `docs/STAGE5C_6A_S_PREREGISTRATION.md`, `docs/STAGE5C_6A_S_RUNNER_FREEZE.md`, `analysis/stage5c_selector_family.py`, `analysis/stage5c_measure_prereg.py`, `analysis/stage5c_6a_s_runner.py`, `tests/test_stage5c_6a_s_runner.py`, `docs/stage5c_development_log.md` |
