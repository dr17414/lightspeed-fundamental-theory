...
| **Stage 5C D.1-3 selector／endpoint contract 路線** | **【提案／v0.12 草案；C8 6a-S prereg 經 pre-execution 修正，整體 feasibility 未證】** | distributional route 已接入 fork B、ambient norm、完整 complexified-real invariant algebra、C3b blind-variety distance form、二維 primary endpoint、wrong-direction gauge 邊界與 11 點 C8 intrinsic selector family。C8 numerical layer 已在不讀 contrast 下固定：$\varphi=1$、$\mathcal N_C=|\Sigma(C)|$、fixed-scale Gaussian regulator、40 complex／80 real nonredundant Fourier signature、同 target S5 metric、S1/S6 floors、$N=64,96,128$、4×64 blocks 與 1536 個 reserved seeds；尚未生成或執行。PR #12 pre-execution audit 已在不碰 reserved streams 下修正結構性 sparse depth member、V-statistic null bias、conjugate redundancy 與 $\epsilon$/boundary claim scope；DEV-0009 再確認 $d_{\rm law}\le0.20$ 只具 gross-breakage tripwire 能力，通過不構成 law stability 證據，並事前登記 `interval_exact(4)`, $N=64$ 的 floor-binding FAIL 風險。6a-E joint-law metric／power／lifecycle、active wrong-support、C6/C7 instances，以及 C3b capability boundary、pair domain／weights、nontriviality／effect-size／noise／continuum calibration 仍未固定。D.1 第 3 項及 Freeze-1a 維持 PENDING，未設計 $K$。 | `docs/STAGE5C_D1_3_OBSERVABLE_CONTRACT.md`, `docs/STAGE5C_SELECTOR_FAMILY.md`, `docs/STAGE5C_6A_S_PREREGISTRATION.md`, `docs/STAGE5C_E3_WRONG_DIRECTION.md`, `docs/STAGE5C_D1_3_PRIMARY_INVARIANT.md`, `docs/STAGE5C_C3B_BLIND_DISTANCE.md`, `analysis/stage5c_measure_prereg.py`, `tests/test_stage5c_measure_prereg.py` |
| **Stage 5C C8 intrinsic selector family** | **【已確認之 candidate-independent 結構交付物／6a-S prereg 修正版完成、execution PENDING】** | $\Sigma_{C8}$ 固定為 sector-blind、unweighted pair-set API，僅接受 `BlindedCase`；有序 family 與 closed capacity 維持 11。pre-execution audit 證明原 `source_depth_band` 把 element-CDF grid 錯移植成 pair-source filter，頂端 band 結構性低於 floor 且破壞 order duality；在 reserved streams 未生成前改為以 $d_-(x)+d_+(y)$ score-level pair-mass midquantile 定義的 `endpoint_depth_mass_band`，五點與 grid 不變。S5 law metric 改 signed unbiased U-statistic；40 complex half-shell 移除 conjugate duplication。DEV-0009 將 $d_{\rm law}$ 的 0.20 gate 明確限縮為 gross-breakage tripwire，並登記 `interval_exact(4)`, $N=64$ 為 floor-binding cell；不改任何 gate、family 或 capacity。這只授權下一段依 commit 一次執行 contrast-free 6a-S；不得記為 PASS、SELECTOR-VIABLE、law stability 或 continuum convergence。 | `docs/STAGE5C_SELECTOR_FAMILY.md`, `docs/STAGE5C_6A_S_PREREGISTRATION.md`, `analysis/stage5c_selector_family.py`, `analysis/stage5c_measure_prereg.py`, `tests/test_stage5c_selector_family.py`, `tests/test_stage5c_measure_prereg.py`, `docs/stage5c_development_log.md` |
